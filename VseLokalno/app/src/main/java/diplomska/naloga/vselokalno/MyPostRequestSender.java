...
public class MyPostRequestSender {
    ...
    public void sendRequest(String mOrderID, String mUserID_to_notify, String mNew_status) throws JSONException {
        JSONObject data = new JSONObject();
//        data.put("ID_from", "");
        data.put("ID_to", mUserID_to_notify);
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest
                (Request.Method.POST, url, requestJson, response -> {
                    makeLogD(TAG, "Got response " + response);
                }, error -> {
                    makeLogW(TAG, "Error! " + error);
                }) {
            @Override
