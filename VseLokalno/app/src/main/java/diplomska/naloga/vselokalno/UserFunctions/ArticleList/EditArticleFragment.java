            ...
            if (photo_changed) {
                try {
                    ...
                    uploadTask.addOnFailureListener(exception -> {
                        ...
                        zaloga = zaloga.trim();
                        if (zaloga.isEmpty())
                            newArticle.put("enota_artikel", "-1");
                        else
                            newArticle.put("enota_artikel", zaloga);
                        appFarm.addArtikel(newArticle);
                        db.collection("Kmetije").document(userID).set(appFarm).addOnCompleteListener(task -> {
                            if (task.isSuccessful()) {
                                makeLogD(TAG, "(onCreateView) updating farm successful!");
                                this.dismiss();
                            } else {
                                makeLogW(TAG, "(onCreateView) updating farm ERROR; task is not successful!");
                            }
                        });
                    });
                } catch (Exception e) {
